(() => {
  ...
  }, $parcel$global.parcelRequire94c2 = parcelRequire), parcelRequire.register("dXGNL", (function(module, exports) {
    var $lkYZ8 = parcelRequire("lkYZ8"), $6U8vl = parcelRequire("6U8vl"), $hnMfj = parcelRequire("hnMfj"), $aCH1p = parcelRequire("aCH1p");
    document.querySelector("button#signout").addEventListener("click", (() => {
      ...
      $parcel$interopDefault($6U8vl).runtime.sendMessage({
        ...
        if ("success" === response.message) {
          let status = "Inactive";
          response.data.profile.status && (status = "Active"), $parcel$interopDefault($lkYZ8)("#name").text(response.data.profile.full_name), 
          $parcel$interopDefault($lkYZ8)("#email").text(response.data.user), $parcel$interopDefault($lkYZ8)("#address").text(response.data.profile.address), 
          $parcel$interopDefault($lkYZ8)("#time").text(response.data.profile.last_time), $parcel$interopDefault($lkYZ8)("#status").text(status);
          let accountType = "Waitlist";
          response.data.profile.is_waitlisted ? $parcel$interopDefault($lkYZ8)(".amount-block").addClass("hidden") : ($parcel$interopDefault($lkYZ8)(".amount-block").removeClass("hidden"), 
          accountType = "Approved", $parcel$interopDefault($lkYZ8)("#paid_amount").text(response.data.profile.paid_amount_text), 
          $parcel$interopDefault($lkYZ8)("#requesting_amount").text(response.data.profile.requesting_amount_text), 
          $parcel$interopDefault($lkYZ8)("#unpaid_amount").text(response.data.profile.unpaid_amount_text), 
          response.data.profile.unpaid_amount >= 1e3 && ($parcel$interopDefault($lkYZ8)(".button-enabled").removeClass("hidden"), 
          $parcel$interopDefault($lkYZ8)(".button-disabled").addClass("hidden"))), $parcel$interopDefault($lkYZ8)("#type").text(accountType);
        } else window.location.replace("../html/popup_sign_in.html");
      })), $parcel$interopDefault($lkYZ8)(".request-payout").click((() => {
        $parcel$interopDefault($lkYZ8).ajax({
          url: $hnMfj.env.guppyApiUrl + "/api/payouts/request/",
          type: "GET",
          dataType: "json"
        }).done((() => {
          $parcel$interopDefault($lkYZ8)(".payout-message").text("Your request is sent"), 
          $parcel$interopDefault($lkYZ8)(".payout-message").addClass("alert-info");
        })).fail((response => {
          $parcel$interopDefault($lkYZ8)(".payout-message").text(response.responseJSON.message), 
          $parcel$interopDefault($lkYZ8)(".payout-message").addClass("alert-danger");
        })), $parcel$interopDefault($lkYZ8)(".request-payout").prop("disabled", !0);
      })), $aCH1p.isAdsReplacerDisabled().then((checked => {
        !1 === checked ? $a2a1fc8fadbd6b85$var$switchButton.checked = !1 : !0 === checked ? $a2a1fc8fadbd6b85$var$switchButton.checked = !0 : document.querySelector(".switch-button-block").style.display = "none";
      }));
    }));
    const $a2a1fc8fadbd6b85$var$switchButton = document.querySelector(".switch-button input");
    $a2a1fc8fadbd6b85$var$switchButton.addEventListener("change", $aCH1p.toggleAdsReplacer);
